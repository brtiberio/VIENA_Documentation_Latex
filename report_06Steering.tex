...
Here is assumed the vehicle follows a model of Ackermann steering model typical valid for low speed which result in the simplified approximation of the single line model or as in general described in literature, as bicycle model.\cite{Snider2009} \cite{Navigation_System_Design} An simplified representation of model is present in figure \ref{fig:ackermann_steering}. The simplified model is normally used in control applications and expected to be used in the high level control. However, it is necessary to describe the sensor readings into angle value of fictional mid wheel. The first step is to identify the range of each physical wheels and therefore the range of fictional wheel. The other step, is done at each vehicle power up and consists in manually perform a full rotation of the steering wheel in each direction until the end is reached.
